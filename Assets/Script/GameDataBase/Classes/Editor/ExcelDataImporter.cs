...
namespace Only1Games.GDBA
{
    public class ExcelDataImporter : EditorWindow
    {
        ...
        {
            string filePath = "Assets/Resources/CSV.data/GameInfoData/";
            switch (_className)
            {
                case "EnemyBaseStat":
                ...
                case "ItemBasePrice":
                    return new ItemBasePriceImporter(filePath);
                case "WeaponBasePrice":
                    return new WeaponBasePiceImporter(filePath);
                case "UpgradeStat":
                    return new UpgradeStatImporter(filePath);
                case "PlayerStat":
                    return new PlayerStatImporter(filePath);
                case "WaveStat":
